...
public class Gen_algoritm : MonoBehaviour
{
    ...
    public float[,] Dist_func =             {{0, 0.5f, 1.5f, 2.5f},         // Тело
                                             {2f, 2.5f, 3f, 4f},            // Близко
                                             {3.5f, 8f, 14f, 15},           // Средне
    ...
    public float population_size = 10;
    // Таймер
    ...
    private void create_population()
    {
        for (int i = 0; i < population_size; i++)
        {
            ...
            float[,] new_func = random_func(Speed_func);
            new_robot.GetComponent<Fuzzy_logic>().Speed_func = new_func;

            new_func = random_func(Dist_func);
            new_robot.GetComponent<Fuzzy_logic>().Dist_func = new_func;

            new_func = random_func(Dist_to_target_func);
            new_robot.GetComponent<Fuzzy_logic>().Dist_to_target_func = new_func;

            new_func = random_func(Deg_to_target_func);
            new_robot.GetComponent<Fuzzy_logic>().Deg_to_target_func = new_func;

            new_func = random_func(Degree_func);
            new_robot.GetComponent<Fuzzy_logic>().Degree_func = new_func;

            Population.Add(new_robot);
    ...
    private float[,] random_func(float[,] speed_func)
    {
        ...
        for (int i = 0; i < ret.GetLength(0); i++)
        {
            ...
            {
                ret[i, j] = speed_func[i, j] + UnityEngine.Random.Range(-max_rand, max_rand);
            }
        }
        return ret;
    }
    ...
    public void read_genetic()
    {
        ...
        for (int i = 0; i < Dist_to_target_func.GetLength(0); i++)
        {
            for (int j = 0; j < 4; j++)
            {
                Gen_vect.Add(Dist_to_target_func[i, j]);
            }
        }
        ...
        for (int i = 0; i < Deg_to_target_func.GetLength(0); i++)
        {
            for (int j = 0; j < 4; j++)
            {
                Gen_vect.Add(Deg_to_target_func[i, j]);
            }
        }
